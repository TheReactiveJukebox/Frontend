...
export class SpeechService {
    ...
    /**
     * Starts browser's speech-search-field recognition and translates spoken words to text using Web Speech Api.
     * @returns Observable<string> The sppech-tp-text result.
     */
    public recordSpeech(): Observable<string> {
        return Observable.create(observer => {
            if (this.initialized) {
                // At the beginning we define functions, which will be called by the speech-search-field api during recognition process

                // do this, when the speech-search-field recognition returns a result
                ...
                // do this, when the speech-search-field recognition fails
                this.recognition.onerror = error => {
                    observer.error(error);
                };

                // do this, after the speech-search-field recognition ends
    ...
    /**
     * Cancels browser's speech-search-field recognition.
     */
